...
function convertInterpretationToJson(task, frames, sourceDocuments) {
  const sourceDocsString = sourceDocuments.map((doc) => ({
    jsonLd: doc.jsonLd,
    selectedSentencesIds: doc.sentences
      .filter((s) => s.selected)
      .map((s) => s.id),
    collapsedSentencesIds: doc.sentences
      ...
      parsedAnnotation.snippets.forEach((parsedSnippet) => {
        ...
        if (parsedSnippet.characterRange[1] > parsedSnippet.characterRange[0]) {
          ...
          const sourceDoc = sourceDocs.find(
            (doc) => doc.id == parsedSnippet.documentId,
          );

          //find sentence for this snippet
          const sentence = sourceDoc.sentences.find(
            (s) => s.id == parsedSnippet.sentenceId,
          );
          //snippet possibly exists, added by another annotation
          let snippet = sentence.snippets.find(
            (s) =>
              s.characterRange[0] == parsedSnippet.characterRange[0] &&
              s.characterRange[1] == parsedSnippet.characterRange[1],
          );
          if (!snippet) {
            ...
            const overlappedSnippetIndex = sentence.snippets.findIndex(
              (s) =>
            ...
            if (
              overlappedSnippet.characterRange[0] < snippet.characterRange[0]
            ) {
              //create snippet left of new snippet
              sentence.snippets.push(
                new Snippet(sentence, [
                  overlappedSnippet.characterRange[0],
                  snippet.characterRange[0],
                ]),
              );
            }
            if (
              overlappedSnippet.characterRange[1] > snippet.characterRange[1]
            ) {
              //create snippet left of new snippet
              sentence.snippets.push(
                new Snippet(sentence, [
                  snippet.characterRange[1],
                  overlappedSnippet.characterRange[1],
                ]),
              );
            }
...
export { convertInterpretationToJson, parseJsonToInterpretation };
