...
function getSelectedCharacterRange(sentenceElement, selection) {
  const indexRangeStart = Math.min(
...
function getHtmlWithHighlights(text, snippets) {
  ...
  snippets = snippets.sort(
    (s1, s2) => s2.characterRange[1] - s1.characterRange[1],
  );
  ...
  snippets.forEach((snippet) => {
    ...
    htmlText =
      htmlText.substring(0, snippet.characterRange[0]) +
      "<span " +
      'id="' +
      snippet.id + //each snippet has a uuid
      '" class="text-white bg-' +
      color +
...
export { getSelectedCharacterRange, getHtmlWithHighlights };
