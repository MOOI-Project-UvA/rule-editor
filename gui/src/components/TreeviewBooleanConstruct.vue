  ...
  methods:{
    ...
    filterFn (val, update, abort) {
       update(()=> {
         ...
         this.options = this.booleanOptions.filter(v=> v.value.toLowerCase().indexOf(needle) > -1)
       })
    },
    ...
    addChild(nodeId){
      ...
      console.log("addedChild to node:", nodeId)
      const selectedData = this.getNodeByKey(nodeId)
      console.log("selected", selectedData)
      selectedData.children.push({
        "frame": null, // if _frame has a value, this BC is 'atomic', it has no children. Its value is a frame.
        "isNegated": false,
        "children": [], // list of BooleanConstructs if _frame is null
        "operatorToJoinChildren": null, //"and" // or "or"
        "parent": null,
        "id": this.maxNumber,
        "label": this.maxNumber,
      })
...
<template>
  <div id="treeview">
    ...
    <q-tree class="q-mt-md" ref="tree-structure" :nodes="sampleStructure" node-key="id" v-model:selected="selectedNode" selected-color="primary" dense default-expand-all>
      <template v-slot:default-header="prop" >
        <div class="row items-center full-width justify-between">
          ...
          <!--            class="q-mr-sm"-->
          <!--          />-->
          <div class="label-node row justify-start">{{ prop.node.label }}</div>
          <div class="boolean-menu row items-center"  v-on:click.stop v-if="prop.node.children.length>0">
            ...
            <q-select
                ...
                @update:model-value="(value)=>{selectValue(value,prop.node.id)}"
                @filter="filterFn"
            >
            ...
            <div class="add-child">
              <q-btn size="sm" dense outline class="q-ml-sm" @click="addChild(prop.node.id)">Add child</q-btn>
            </div>
          </div>
          <div v-else>
            <q-btn size="sm" dense outline class="q-ml-sm" @click="addChild(prop.node.id)">Add child</q-btn>
      ...
      <template v-slot:default-body="prop">
          <div class="panel flex flex-row" v-if="prop.node.children.length === 0" :class="{negated: prop.node.isNegated}">
            <div class="col">
              <!-- negation label -->
            ...
            <div class="col-1 buttons-container">
               <div>
                 <q-btn size="sm" color="#d42d19" dense flat icon="mdi-minus-circle-outline" @click="toggleNegation(prop.node.id)" />
               </div>
