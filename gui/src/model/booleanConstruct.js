...
export class BooleanConstruct {
  ...
  get operatorToJoinChildren() {
    return this._operatorToJoinChildren;
  }
  set operatorToJoinChildren(operator) {
    this._operatorToJoinChildren = operator;
  }
  ...
  toFlatObject() {
    console.log("bConstruct to FlatObject:");
    return {
      frame: this.frame?.id,
  ...
  fromFlatObject(bcData, allFrames) {
    console.log("fromFlatObject:", bcData);
    this._frame = bcData.frame
    ...
    // TODO: Does it come from fact or act?
    // add the extra level to render the fact correctly/
    if (this._children.length === 0) this.subdivide();
  }
  ...
  toString() {
    ...
    if (this.frame) {
      ...
      s += this.children.map((child) => "\t" + child.toString()).join("\n");
    }
    return s;
  }
}
