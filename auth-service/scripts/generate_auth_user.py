    ...
    for raw_line in env_file.read_text(encoding="utf-8").splitlines():
        ...
        if isinstance(parsed, dict):
            for username, password_hash in parsed.items():
                ...
                normalized_hash = str(password_hash).strip().replace("$$", "$")
...
def write_auth_users_to_env_file(env_file: Path, users: dict[str, str]) -> None:
    ...
    serialized_for_compose_env = serialized.replace("$", "$$")
    auth_line = f"AUTH_USERS_JSON={serialized_for_compose_env}"
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "argon2 is not installed. Run `pip install -r requirements.txt` in auth-service first."
        ) from exc
